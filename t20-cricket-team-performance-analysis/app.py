...
# -------------------------------------------------
# FOOTER FUNCTION
# Uses components.html() instead of st.markdown()
# to bypass Streamlit's markdown parser which was
# incorrectly rendering the flex HTML as a code block.
# -------------------------------------------------
    ...
    components.html(
        ...
        <style>
            ...
            .footer-credit {
                ...
                color: #9aa0a6;
                font-size: 14px;
                padding: 12px 0 5px 0;   /* extra space so text isn't clipped */
            }
        </style>
        ...
        """,
        height=200,   # increased height so nothing is cropped
    )
    ...
    if page == "Analyze your Dataset":  #🏏
        ...
        with tab1:
            ...
            with st.expander("🙎 Match & Player Info", expanded=False):
                ...
                column_info("Player_Name", "Enter the name of the player whose statistics are being recorded. Null values are NOT allowed when other player statistics are present. Preferred datatype: String.")
        ...
        with tab4:
            ...
            if st.session_state.get("df2") is None:
                ...
                st.markdown(
                    f"""
**1.** **{runs_difference_s.iloc[0]["Player_Name"]}** is the highest run scorer with **{runs_difference_s.iloc[0]["Runs_Scored"]} runs**.

**2.** **{bowling_diff_s.iloc[0]["Player_Name"]}** is the highest wicket taker with **{bowling_diff_s.iloc[0]["Wickets_Taken"]} wickets**.

**3.** **{strike_rate_diff_s.iloc[0]["Player_Name"]}** has the highest strike rate of **{strike_rate_diff_s.iloc[0]["Strike_Rate"]}**.

**4.** **{economy_diff_s.iloc[0]["Player_Name"]}** has the best bowling economy rate of **{economy_diff_s.iloc[0]["Economy_Rate"]}**.

**5.** **{runs_contribution_by_role_s.iloc[0]["Role"]}** are the highest contributors in scoring total runs with **{runs_contribution_by_role_s.iloc[0]["Runs_Scored"]} runs**, followed by **{runs_contribution_by_role_s.iloc[1]["Role"]}** with **{runs_contribution_by_role_s.iloc[1]["Runs_Scored"]} runs**.

**6.** **{wickets_contribution_by_role_s.iloc[0]["Role"]}s** are the highest contributors in taking wickets with **{wickets_contribution_by_role_s.iloc[0]["Wickets_Taken"]} wickets**, followed by **{wickets_contribution_by_role_s.iloc[1]["Role"]}s** with **{wickets_contribution_by_role_s.iloc[1]["Wickets_Taken"]} wickets**.

**7.** The **{order_runs_s.iloc[0]["Batting_Order"]} order** is the strongest batting order followed by the **{order_runs_s.iloc[1]["Batting_Order"]} order**.

**8.** The team has lost most wickets in the **{phase_wickets_s.iloc[0]["Wicket_Phase"]}**, losing **{phase_wickets_s.iloc[0]["Wickets_Lost"]} wickets**.
                    """
                )
...
# -------------------------------------------------
# FOOTER
# Rendered outside all page/column/tab blocks.
# Uses components.html() to bypass Streamlit's
# markdown parser which was rendering flex HTML
# as a code block instead of actual HTML.
# -------------------------------------------------
